...
const data = [
    ...
    },
    {
        ...
        type: 'latest',
        description: 'Foods to eat when you have eaten too much salt, Have you ever imagined what happens when you have too much salt in your body? ',
    ...
    },
    {
        ...
        date: ' Bollywood/ August 21 2017',
        id: '19',
        explanation: 'The upcoming Telugu film Godfather, which features megastar Chiranjeevi, will have an appearance from Bollywood superstar Salman Khan.The movie is a Telugu remake of the Malayalam blockbuster Lucifer, which starred Mohanlal in the lead role.Lucifer also marked the directorial debut of actor Prithviraj.Salman will reportedly essay the role played by Prithviraj in the film with certain modifications.According to the home reports, Salman and Chiranjeevi will start shooting for the film in Hyderabad by January end or home by February first week. Reportedly, Salman plays a dynamic role and he has allotted 5 to 7 days for the cameo. The makers have made modifications from the original film to curate several larger-than-life moments keeping Salman aura in mind.'
...
const  categoryHandler= (req,res) => {
    ...
    if(req.params.category){
        detail=data.filter((item) => item.category === req.params.category)
...
const idHandler=(req,res)=>{
    ...
    if (req.params.id && req.params.category) {
        detail = data.find((item) => item.id === req.params.id && item.category === req.params.category)
        res.send(detail)
